...
const EditorialsPage: React.FC = () => {
    ...
    const { data: articles = [], isLoading: isLoadingArticles } = useQuery({
        queryKey: ['articles', 'Editorial', language],
        queryFn: () => fetchArticles('Editorial', language)
    });
        ...
        <div className="min-h-screen bg-white font-body">
                ...
                <div className="container mx-auto px-6 md:px-12 relative z-10 text-center max-w-5xl">
                    ...
                    >
                        <div className="inline-flex items-center gap-3 px-6 py-2 bg-slate-50 rounded-full mb-8 md:mb-10 border border-slate-100">
                            ...
                            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-slate-700">{t('updated')} Jan 2026</span>
                        </div>
                        <h1 className="font-heading text-6xl md:text-9xl font-black text-primary leading-[0.85] mb-8 lg:mb-12 tracking-tighter">
                            The <span className="text-accent">{t('editorials')}.</span>
                        </h1>
            ...
            <main className="container mx-auto px-6 md:px-12 py-16 md:py-24">
                        ...
                        <Link to={`/journal/${topStory.id}`} data-cursor="read">
                            ...
                            <div className="mt-8 md:mt-12 max-w-3xl">
                                <div className="flex items-center gap-3 text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-accent mb-6 lg:mb-8">
                                    <span className="px-3 md:px-4 py-1.5 md:py-2 bg-accent/10 rounded-full">Featured Editorial</span>
                                    <span>•</span>
                                    <span className="text-slate-600 font-bold">{topStory.readTime} Deep Dive</span>
                                </div>
                                ...
                                <p className="text-lg md:text-2xl text-slate-700 font-bold leading-relaxed mb-8 md:mb-10 line-clamp-2 italic">
                                    "{topStory.excerpt}"
                                </p>
                                <div className="flex items-center gap-4 text-primary font-black uppercase text-[9px] md:text-[10px] tracking-widest pb-2 transition-all group-hover:gap-8 border-b-2 border-transparent hover:border-accent w-fit">
                                    Enter Perspective <ArrowRight size={18} className="text-accent" />
                                </div>
                            </div>
                        </Link>
                    </motion.div>
                    ...
                    >
                        <div className="p-8 md:p-12 bg-[#FBFBFF] rounded-[40px] md:rounded-[60px] border border-slate-50 h-full">
                            ...
                            <div className="space-y-10 lg:space-y-12">
                                {recentArticles.filter(a => a.id !== topStory.id).slice(0, 4).map((item, idx) => (
                        ...
                        {articles.map((art, idx) => (
                            ...
                            >
                                <Link to={`/journal/${art.id}`} className="group block h-full">
                                    ...
                                    <div className="px-2">
                                        <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] text-accent mb-3 lg:mb-4 block">Ref No. E-{idx + 104}</span>
                                        ...
                                        <p className="text-sm lg:text-base text-slate-500 font-medium leading-relaxed line-clamp-3 mb-8 italic">
                                            "{art.excerpt}"
                                        </p>
                ...
                >
                    ...
                    <PenTool className="mx-auto text-slate-200 mb-6 lg:mb-8 w-8 h-8 lg:w-10 lg:h-10" />
                    <h3 className="font-heading text-3xl md:text-4xl font-black text-primary mb-10 lg:mb-12 italic tracking-tight">Keep Exploration Alive.</h3>
                    <button className="px-10 py-5 bg-primary text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-2xl hover:bg-accent transition-all">
