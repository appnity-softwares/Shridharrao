...
import { fetchArticles, fetchHeadlines, fetchAds } from '../services/api';
...
const StoriesPage: React.FC = () => {
    ...
    const { data: articles = [], isLoading: isLoadingArticles } = useQuery({
        queryKey: ['articles', 'Story', language],
        queryFn: () => fetchArticles('Story', language)
    });
    ...
    const loading = isLoadingArticles || isLoadingHeadlines;

    if (loading) return <div className="min-h-screen bg-white flex items-center justify-center">Loading...</div>;

    const topStory = articles[0]; // First retrieved is top
    // Handle empty state
    if (!topStory && articles.length === 0) return <div className="min-h-screen bg-white flex items-center justify-center">No stories found.</div>;

    return (
        <div className="min-h-screen bg-white font-body">
            {bannerAd && <AdBanner ad={bannerAd} />}
            <AdModal ads={ads} />
            {/* Minimalist Interactive Header */}
                ...
                <div className="container mx-auto px-6 md:px-12 relative z-10 text-center max-w-5xl">
                    ...
                    >
                        <div className="inline-flex items-center gap-3 px-6 py-2 bg-slate-50 rounded-full mb-8 md:mb-10 border border-slate-100">
                            ...
                            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Updated Jan 2026</span>
                        </div>
                        <h1 className="font-heading text-6xl md:text-9xl font-black text-primary leading-[0.85] mb-8 lg:mb-12 tracking-tighter">
                            The <span className="text-accent">{t('stories')}.</span>
                        </h1>
            ...
            <main className="container mx-auto px-6 md:px-12 py-16 md:py-24">
                        ...
                        <Link to={`/journal/${topStory.id}`} data-cursor="read">
                            ...
                            <div className="mt-8 md:mt-12 max-w-3xl">
                                <div className="flex items-center gap-3 text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-accent mb-6 lg:mb-8">
                                    <span className="px-3 md:px-4 py-1.5 md:py-2 bg-accent/10 rounded-full">Featured Story</span>
                                    <span>•</span>
                                    ...
                                    <Link key={item.id} to={`/journal/${item.id}`} className="block group" data-cursor="read">
                                        <div className="flex gap-5 md:gap-6 items-start">
                                            ...
                                            <div className="flex-1 border-b border-slate-100 pb-6 lg:pb-8 group-last:border-0">
                                                <h4 className="text-base md:text-lg font-bold text-primary leading-snug mb-2 group-hover:text-accent transition-colors">
                                                    {item.title}
                                                </h4>
                                                <span className="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-[0.2em]">{item.time} Ago</span>
                                            </div>
                                        </div>
                                    </Link>
                                ))}
                        ...
                        {Array.isArray(articles) && articles.map((art, idx) => (
                            ...
                            >
                                <Link to={`/journal/${art.id}`} className="group block h-full">
                                    ...
                                    <div className="px-2">
                                        <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] text-accent mb-3 lg:mb-4 block">Ref No. {idx + 304}</span>
                ...
                >
                    ...
                    <BookOpen className="mx-auto text-slate-200 mb-6 lg:mb-8 w-8 h-8 lg:w-10 lg:h-10" />
                    <h3 className="font-heading text-3xl md:text-4xl font-black text-primary mb-10 lg:mb-12 italic tracking-tight">Every face has a story.</h3>
